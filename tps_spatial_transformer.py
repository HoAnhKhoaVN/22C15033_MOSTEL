...
class TPSSpatialTransformer(nn.Module):
    ...
    def _interpolate(self, im, x, y):
        ...
        height_f = torch.tensor(height, dtype=torch.float32 ,device=cuda0)
        width_f = torch.tensor(width, dtype=torch.float32 ,device=cuda0)
        zero = torch.tensor(0, dtype=torch.int32 ,device=cuda0)
        max_y = torch.tensor(height - 1, dtype=torch.int32 ,device=cuda0)
        max_x = torch.tensor(width - 1, dtype=torch.int32 ,device=cuda0)
